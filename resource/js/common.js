(function () {
  ...
  /* ── 풀페이지 스크롤 (fp-wrapper가 있는 페이지에서만 동작) ──
     edu.html(일반 스크롤), edu-detail.html(자체 scrollIntoView 방식)처럼
     #fp-wrapper가 없는 페이지에서 이 블록이 그대로 실행되면
     - wheel/touchmove의 preventDefault가 정상 스크롤을 막아버리고
     - wrapper.style.transform 에서 null 참조 에러가 나는 문제가 있어
     존재 여부로 감싸 안전하게 만듦. */
  if (wrapper && totalSections) {
    ...
    /* ── Core: go to section ────────────────────────────────── */
    // vh 대신 실제 섹션 높이(px)를 측정해서 이동거리를 계산.
    // 모바일 브라우저(아이폰16, 갤럭시S24 등)는 스크롤 중 주소창이
    // 접혔다 펼쳐지면서 100vh 값이 실시간으로 바뀌는데,
    // 섹션 자체는 CSS의 100svh로 크기가 잡혀있어서
    // "이동 거리(vh) ≠ 섹션 실제 높이(svh)" 불일치가 생겨 화면이
    // 어긋나거나 끊겨 보이는 문제가 있었음. 픽셀 실측으로 통일해 해결.
